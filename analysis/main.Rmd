...
__Regression analysis__ is a method that uncovers the average value of a variable _y_ for different values of variable _x_. Regression is a model for the __conditional mean__. The model for the conditional mean gives a rule that takes _x_ values and tells what the mean value of _y_ is. $$\tag{The expected value of y conditional on x is given by the function f} E[y|x] = f(x) \\ y^E = f(x)$$ $$\tag{The actual value of y is equal to its expected value plus a deviation from it}  y = f(x) + e$$.
